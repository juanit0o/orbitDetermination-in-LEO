...
def main():
    ...
    train_dataloader = trainingData.to_dataloader(train=True, batch_size=batch_size, num_workers=10)
    ...
    print("=================================")
    best_model_path = "C:/Users/jpfun/Desktop/Gitlab Neuraspace/joao-funenga/src/callbacksTFT/lr_find_5a9369fb-17b0-4080-adc0-6dff63fd165a.ckpt"
    ...
    #print first raw prediction shape
    print(np.shape(raw_predictions['prediction'][0])) #torch.Size([197, 20, 7]) [nr different orbit, max_prediction_length?, output_size]
    print(np.shape(raw_predictions['encoder_attention'][0])) #torch.Size([20, 1, 10]) [max_prediction_length, attention_head_size, encoder_length]
    for idx in range(3,4): #only for index = 4
        print(f"Plotting example {idx}")
        #retorna lista de fotos
        listFigures = best_tft.plot_prediction(x, raw_predictions, idx=idx, add_loss_to_title=True)
        print(len(listFigures)) # 9 variables to predict
        targets = list(trainingData.target)
        for i in range(len(listFigures)):
            plt.title(targets[i])
            listFigures[i].savefig(f"fig{idx}_{i}.png")
            plt.show()
            plt.figure()
